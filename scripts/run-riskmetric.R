...
# When TARGET_PACKAGES is set (comma-separated), score only those packages.
# Used by validation-gate.yml to score only changed packages.
target_pkgs_env <- Sys.getenv("TARGET_PACKAGES", "")
if (nchar(target_pkgs_env) > 0) {
  keep <- trimws(strsplit(target_pkgs_env, ",")[[1]])
  all_pkgs <- intersect(all_pkgs, keep)
}
